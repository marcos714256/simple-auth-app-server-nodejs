...
const login = async (req, res) => {
    ...
    const userFound = await auth_1.User.findOne({ email });
    if (!userFound) {
...
const verifyAccessToken = async (req, res) => {
    ...
    jsonwebtoken_1.default.verify(userToken, config_1.SECRET_KEY, async (error, user) => {
        ...
        const userFound = await auth_1.User.findById(user.userId);
        if (!userFound)
...
const resetPassword = async (req, res) => {
    ...
    try {
        ...
        const userFound = await auth_1.User.findById(decoded.userId);
        if (userFound !== null) {
        ...
        try {
            await auth_1.User.findByIdAndUpdate(decoded.userId, { password: passwordHash }, { new: true });
